...
retail = pd.read_excel(data_path).rename(columns=column_names)  # <1>

```

1. A renomeação acontece na leitura para que todo o restante da análise use a convenção Python `snake_case`, sem alterar o arquivo original.
...
3. Existe alguma linha com unit_price igual a zero e customer_id ausente ao mesmo tempo? (Dica: combine um filtro de igualdade com o método isna() da coluna.)
