---
title: "Aula 1 - Ferramentas, Ambiente e Versionamento"
...
1. Descrever o ecossistema de ferramentas do cientista de dados que trabalha com Python e o papel de cada componente;
2. Conhecer as principais IDEs usadas em ciência de dados, em Python e em outras linguagens;
3. Criar a estrutura de um projeto de dados pelo terminal;
4. Explicar a importância do versionamento de código e executar o ciclo essencial do Git (init, status, add, commit, log), sabendo que também existem tecnologias para versionamento de dados e de modelos;
5. Comparar os gerenciadores de pacotes e ambientes mais comuns (pip, venv, conda e uv) e escolher o adequado para cada situação;
6. Criar e usar ambientes virtuais isolados, explicando por que o isolamento e o registro de dependências são essenciais para a reprodutibilidade.
...
```{python}
# Sua primeira celula de codigo: vamos verificar a versao do Python.
import sys

print("Versao do Python:", sys.version)
print("Executavel em uso:", sys.executable)
```
...
Uma nota sobre o formato: nas células deste roteiro, o ponto de exclamação no início da linha indica que o comando é de terminal, e não código Python. Assim tudo fica registrado no próprio documento. No dia a dia, você digitará os mesmos comandos diretamente no terminal, com efeito idêntico. Repetimos o cd em cada célula porque cada célula abre uma sessão nova de terminal; no terminal de verdade, você entra na pasta uma vez e trabalha dentro dela.

```{python}
# Criamos a pasta do projeto e uma estrutura minima:
# data/ para dados, notebooks/ para analises, src/ para codigo reutilizavel.
!mkdir -p ./projeto-vendas/data ./projeto-vendas/notebooks ./projeto-vendas/src
...
O git init criou uma pasta oculta chamada .git, onde todo o histórico ficará guardado. A partir de agora, o Git observa o projeto, mas nunca registra nada sem um comando explícito seu. Vamos criar o primeiro arquivo e perguntar ao Git o que ele está vendo:
...
Observe a saída com atenção: o Git enxergou o README.md, mas o classificou como untracked, ou seja, não rastreado. Ele está avisando que existe um arquivo novo, mas que você ainda não disse se ele deve entrar na próxima fotografia. O Git nunca decide isso por você. Vamos decidir:
...
- Em março, você desenvolveu o Projeto A, que utiliza pandas na versão 1.5.
- Em agosto, você começou o Projeto B e atualizou o pandas para a versão 2.2.
- Em setembro, você reabriu o Projeto A e ele quebrou: a atualização mudou comportamentos dos quais o código antigo dependia.
...
Repare no hábito se formando: cada etapa do projeto termina com um commit. É o segundo do dia, e não será o último.
...
Observe a saída do uv pip list: cada biblioteca aparece acompanhada de um número de versão exato, por exemplo pandas 2.2.3, além das dependências que vieram junto, como o numpy. Esse detalhe é o coração da reprodutibilidade, porque bibliotecas mudam de comportamento entre versões: uma função pode ser renomeada, um parâmetro pode mudar de valor padrão, um bug pode ser corrigido, alterando o resultado numérico, ou introduzido. O mesmo código com pandas 1.5 e com pandas 2.2 pode produzir resultados diferentes, ou simplesmente não rodar. Dizer "usei pandas" não basta; é preciso registrar "usei pandas 2.2.3".
...
Uma confissão para fechar a seção: fizemos cada passo manualmente, de propósito, para que você entendesse cada peça. No dia a dia, o uv automatiza boa parte disso: o comando uv init cria a pasta, o pyproject.toml (a descrição moderna do projeto) e o ambiente de uma vez, e o uv add pandas instala a biblioteca já registrando a versão exata em um arquivo de lock (uv.lock), mais preciso que o requirements.txt. A lógica, porém, é idêntica: criar, isolar, instalar, registrar. Você aprendeu o mecanismo; a automação agora é um atalho consciente, não uma caixa-preta. Experimente o uv init no exercício para casa.
...
1. "Adiciona README com a descricao do projeto": o projeto nasceu e ganhou memória;
2. "Adiciona .gitignore para ambiente, dados e segredos": o ambiente virtual chegou, causou o incidente do git status e ensinou a diferença entre o que se versiona e o que se ignora;
3. "Adiciona pandas e registra dependencias em requirements.txt": o projeto ganhou a sua primeira dependência, com a versão devidamente registrada.
...
- Conheceu o mapa do ecossistema do cientista de dados e as principais IDEs da área, em Python e em outras linguagens;
- Criou um projeto do zero pelo terminal, com estrutura de pastas;
- Deu memória ao projeto com o Git e praticou o ciclo essencial (init, status, add, commit, log) três vezes, uma a cada etapa;
- Comparou pip, venv, conda e uv, e sabe que o uv é o padrão atual para projetos novos, que o pip é o fundamento universal e que o conda é a escolha para dependências científicas nativas;
- Criou um ambiente virtual isolado, viveu o incidente do .venv no git status e o resolveu com o .gitignore;
- Materializou a fórmula da reprodutibilidade: isolamento mais registro de versões, com o requirements.txt versionado no repositório;
- Descobriu que dados e modelos também são versionados, com ferramentas como DVC e MLflow, e que branch, merge e GitHub são os próximos capítulos.
...
3. Instalar as dependências registradas, com uv pip install -r requirements.txt.
...
1. Refaça o projeto do zero, em uma pasta nova e sem consultar o roteiro, digitando os comandos diretamente no terminal, sem o ponto de exclamação. Se errar, ótimo: errar aqui é barato e pedagógico. Use o git status como bússola sempre que se perder.
2. Experimente a automação: crie um segundo projeto com uv init, adicione o pandas com uv add pandas e compare os arquivos gerados (pyproject.toml e uv.lock) com o que fizemos manualmente. Tente identificar o que cada arquivo registra.
3. Crie uma conta no GitHub, caso ainda não tenha. Na próxima aula vamos publicar o projeto-vendas lá e conhecer clone, push, pull, branch e merge.
